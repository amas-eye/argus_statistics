...
def update_health(health: int):
    ts = int(time.time())
    send_data = {"check_time": ts, "score": health}
...
def get_host_by_system():
    ...
    if result:
        ...
        param = {
            "start": "{interval}s-ago".format(interval=interval_time),
            "m": "max:{metric}{add}".format(metric=metric_name, add="{host=*}")
        }
